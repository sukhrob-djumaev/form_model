...
void main() {
  group('FormModelLocalizations Comprehensive Tests', () {
    ...
    test('Setting unsupported locale should throw ArgumentError', () {
      expect(() => localizations.currentLocale = const Locale('xx'),
    ...
    group('Error key translations for all supported languages', () {
      for (var locale in supportedLocales) {
        test('$locale translations', () {
          ...
          expect(
              localizations.translateErrorKey(const PredefinedFormErrorKey(
          ...
          var containsTranslation = localizations.translateErrorKey(
              const PredefinedFormErrorKey(
                  PredefinedFormErrorType.stringDoesNotContain, '@'));
          expect(containsTranslation, contains('@'));

          // Test file size formatting
          var fileSizeTranslation = localizations.translateErrorKey(
              const PredefinedFormErrorKey(
                  PredefinedFormErrorType.fileSizeExceedsLimit, 1048576));
          expect(
            fileSizeTranslation,
            contains('1'),
          );
    ...
    test('Custom error translations', () {
      ...
      for (var locale in supportedLocales) {
        localizations.setCustomErrorTranslations(
            locale, customKey, 'Custom error in $locale');
        localizations.currentLocale = Locale(locale);
        expect(localizations.translateErrorKey(customKey),
            equals('Custom error in $locale'));
      }
    });
    ...
    test('Consistent translations across languages', () {
      const testCases = [
        ...
        PredefinedFormErrorKey(
            PredefinedFormErrorType.stringDoesNotContain, '@'),
        PredefinedFormErrorKey(
            PredefinedFormErrorType.invalidFileType, ['jpg', 'png', 'gif']),
      ];
      ...
      for (var errorKey in testCases) {
        ...
        expect(
            translations.values.toSet().length, equals(supportedLocales.length),
    ...
    test('Interpolation of parameters', () {
      for (var locale in supportedLocales) {
        localizations.currentLocale = Locale(locale);
        var errorKey = const PredefinedFormErrorKey(
            PredefinedFormErrorType.lengthIsLessThanMin, 10);
        var translation = localizations.translateErrorKey(errorKey);
