...
/// match the pattern, it returns a [FormError] with the [ErrorCode.didNotMatchPattern]
...
final class CustomPatternValidator implements BaseFormModelValidator<String?> {
  ...
  /// does not match the [pattern], it returns a [FormError] with the [ErrorCode.didNotMatchPattern]
  ...
  FormError<String?>? validate(String? value) {
    if (value != null) {
      if (!pattern.hasMatch(value)) {
        return FormError(code: ErrorCode.didNotMatchPattern, value: value, parameter: pattern);
      }
    }
    return null;
  }
