...
class PredefinedFormErrorKeyTranslationsZhCn implements PredefinedFormErrorKeyTranslations {
  @override
  String translate(PredefinedFormErrorKey errorKey) {
    return switch (errorKey.type) {
      PredefinedFormErrorType.required => '此字段为必填项。',
      PredefinedFormErrorType.lengthIsLessThanMin => '长度应至少为 ${errorKey.parameter}。',
      PredefinedFormErrorType.lengthIsMoreThanMax => '长度应最多为 ${errorKey.parameter}。',
      PredefinedFormErrorType.lengthIsNotEqual => '长度应恰好为 ${errorKey.parameter}。',
      PredefinedFormErrorType.didNotMatchPattern => '值不符合预期模式：${errorKey.parameter}。',
      ...
      PredefinedFormErrorType.dateIsLessThanMinAge => '值应至少为 ${errorKey.parameter}。',
      PredefinedFormErrorType.dateIsMoreThanMaxAge => '值应最多为 ${errorKey.parameter}。',
      PredefinedFormErrorType.numIsLessThanMin => '值应至少为 ${errorKey.parameter}。',
      PredefinedFormErrorType.numIsMoreThanMax => '值应最多为 ${errorKey.parameter}。',
      ...
      PredefinedFormErrorType.wordCountIsLessThan => '字数应至少为 ${errorKey.parameter}。',
      PredefinedFormErrorType.wordCountIsMoreThan => '字数应最多为 ${errorKey.parameter}。',
      ...
      PredefinedFormErrorType.passwordTooShort => '密码长度至少应为 ${errorKey.parameter} 个字符。',
      ...
      PredefinedFormErrorType.passwordNoSpecialChar => '密码必须包含至少一个特殊字符 (${errorKey.parameter})。',
      PredefinedFormErrorType.stringDoesNotContain => '输入必须包含 "${errorKey.parameter}"。',
      PredefinedFormErrorType.stringContains => '输入不得包含 "${errorKey.parameter}"。',
      PredefinedFormErrorType.invalidFileType =>
        errorKey.parameter is List ? '无效的文件类型。允许的类型是：${(errorKey.parameter as List<String>).join("、")}。' : '无效的文件类型。',
      PredefinedFormErrorType.fileSizeExceedsLimit => '文件大小超过最大限制${_formatFileSize(errorKey.parameter)}。',
    };
  }
